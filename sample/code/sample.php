...
<?php
    ...
    if ($topicCode == NULL || $cat_or_tag == NULL || !in_array($topicCode, $topic_codes)) {
        ...
        foreach($topic_codes as $topic_code) {
            echo '<a href="javascript:void(0)" onclick=window.open("sample.php?topic='. $topic_code . '&cat_or_tag=category")>' . $topic_code . '</a><br>';
            if($i++ == 12) {
                break;
            }
        }
        ...
        foreach($topic_codes as $topic_code) {
            if($i++ > 12) {
                echo '<a href="javascript:void(0)" onclick=window.open("sample.php?topic='. $topic_code . '&cat_or_tag=tag")>' . $topic_code . '</a><br>';
            }
        }
    }
